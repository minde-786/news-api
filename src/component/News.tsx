...
function News() {
  ...
  const getData = async () => {
    try {
      if (!NEXT_PUBLIC_NEWS_API_KEY) throw new Error("API key is missing")
      const res = await fetch(`https://newsapi.org/v2/everything?q=${search}&apiKey=${NEXT_PUBLIC_NEWS_API_KEY}` 
  ...
  return (
    <div>
      <nav className="bg-slate-100 shadow-lg px-4 py-3 flex flex-col gap-3 lg:grid lg:grid-cols-3 lg:gap-4 lg:items-center">
        ...
        <div className="flex justify-center gap-6">
          ...
          <Link
            ...
            className={`cursor-pointer ${
              activeTab === "trending" ? "font-bold bg-amber-400 rounded px-2 py-1" : ""
            }`}
          >
            Trending
          </Link>
      ...
      <h1 className="m-4 text-2xl text-center font-semibold">Stay Updated with Trendy News</h1>

      <div className="flex justify-center lg:gap-16 mt-8 gap-2 px-2">
        {["sport", "Health", "Entertainment", "Fitness", "Politics"].map((cat) => (
          <Link
            href="#"
            key={cat}
            onClick={() => userInput(cat)}
            className="bg-red-500 rounded-lg px-2 py-1 w-32 text-center"
          >
            {cat}
          </Link>
        ))}
